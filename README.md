...
Create environment files in `frontend/` (not committed):
...
npm run start:staging    # staging (requires .env.staging)
npm run start:production # production (requires .env.production)
...
See `docs/Configuration.md` and `docs/API.md` for details.
